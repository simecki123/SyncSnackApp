...
export default function OrdersManagement({ orders, accessToken }: { orders: any[], accessToken: string }) {
  ...
  const renderView = () => {
    switch (view) {
      case 'table':
        return <OrdersTable setCurrentPage={0} accessToken={accessToken} orders={orders} currentPage={0} />;
      case 'column':
        return <Statistics stats={calculateStatistics(orders)} />;
      case 'pie':
        return <PieChart data={orders} option={option} setOption={setOption} />;
      default:
        return <OrdersTable setCurrentPage={0} orders={orders} accessToken={accessToken} currentPage={0} />;
    }
  };
